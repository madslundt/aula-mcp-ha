...
const INSTRUCTIONS = [
  ...
  "   matches `Lukas`). Always read IDs from the manifest — never guess:",
  '   `children[].id` is the childId; `children[].institution.id` is the',
  '   calendar profileId; `children[].institution.code` is the institutionCode;',
  '   `manifest.institutionProfileIds` is the GUARDIAN scope for posts.list.',
  "   See `usage.nameResolution` on the manifest for the full mapping.",
